...
import static cn.com.helei.application.unich.UnichClaimBot.TOKEN_EXPIRE_TIME_KEY;

@Slf4j
public class UnichApi {

    private final UnichClaimBot bot;

    private final UnichConfig unichConfig;

    public UnichApi(UnichClaimBot bot) {
        this.bot = bot;
        this.unichConfig = (UnichConfig) bot.getBaseAutoBotConfig();
    }
     ...
     */
    public CompletableFuture<String> loginAndTakeToken(AccountContext accountContext) {
     ...
     * @return boolean
     */
    public boolean startMining(AccountContext accountContext) {
        ...
        bot.syncRequest(proxy, listUrl, "post", headers, null, new JSONObject(),
                ...
                }, bot.getExecutorService()).exceptionally(throwable -> {
    ...
    public String startClaimSocialReward() {
        Semaphore semaphore = new Semaphore(unichConfig.getConcurrentCount());
        ...
        return "已开始领取账户社交奖励任务, 共[{}]" + bot.getAccounts().size() + "个账户";
    }
    ...
    private CompletableFuture<List<String>> queryAccountUnClaimedTaskIds(NetworkProxy proxy, Map<String, String> headers, String printPrefix) {
        ...
        return bot.syncRequest(proxy, listUrl, "get", headers, null, null,
    ...
    private List<String> claimAccountTasks(AccountContext accountContext, List<String> unclaimedTaskId, Map<String, String> headers) {
        ...
        List<CompletableFuture<String>> subTaskFutures = unclaimedTaskId.stream().map(taskId -> {
            ...
            return bot.syncRequest(proxy, claimUrl + taskId, "post", headers, null, body,
